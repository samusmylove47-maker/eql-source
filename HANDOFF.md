...
| ~~Migrating every internal href to the extensionless form~~ **DONE 1 Sep 2026** | All 33,732 internal page links are extensionless; the 4 that are not are the hashed app bundles, which have no extensionless form. The 29 `_redirects` targets went with them, so a shared short link no longer pays a second hop. **The `.html` files are untouched on disk and the 307 stays**, so every URL already in the wild still works — measured, 0 previously-published addresses lost. **The cost nobody had written down: local `file://` browsing no longer navigates.** `/tools/lockouts` gives *"Your file couldn't be accessed"* and `/dungeons/` gives a directory listing, both measured in Chrome. `conformance.js` is unaffected — it opens each page by path and never follows a link. |
