...
WHAT IS NOT PUBLISHED, AND WHAT THAT DOES AND DOES NOT MEAN
-----------------------------------------------------------
Drops whose item is not in our catalogue are held back rather than published.

**THIS PARAGRAPH USED TO CALL THEM VENDOR TRASH AND SAY THE CATALOGUE WAS RIGHT
TO OMIT THEM. Both halves were withdrawn on 4 Sep 2026 and this text did not
move until 6 Sep.** Nothing here tests whether an item is trash. What it tests
is whether the item is in our catalogue, which is a fact about OUR COVERAGE and
not about the item - and under the old label that covered hundreds of drops from
mobs our own roster names. See `uncatalogued_drops_excluded` below, which was
called `excluded_trash_drops` for the same reason and was renamed with it.

So: not published, not counted on the pages, and NOT evidence either way about
whether the catalogue should hold them. `uncatalogued_by_zone` exists so the
question can be asked per zone, where it can actually be acted on.

The counts are deliberately absent from this paragraph. main() prints them on
every run, and a figure typed beside the data it describes is the fault
CLAUDE.md section 3 names - which is how the sentence above survived its own
retraction by two days.
...
def main():
    ...
    out = {
        '_comment': [
            ...
            'Drops whose item is not in our catalogue are excluded. That is a '
            'fact about our coverage, not a judgement about the item.',
