...
ENTRIES = [
    ...
    dict(kind="Correction", date="14 Aug 2026", title="We broke the Plane of Sky tracker for a day",
         body="Moving the Sky dataset out of the page and into <code>sky.json</code> took one constant with it that nothing noticed: <code>ORDER</code>, the class picker&rsquo;s display order. It sat just past the block that moved. <strong>The picker rendered no classes at all</strong>, so a trio could never reach three and <em>Build character sheet</em> stayed permanently greyed out. Reported by the collaborator, who was right to think it looked broken. <strong>All 721 pages passed our own checks while the tool was unusable</strong>, because every check we had reads the HTML a page ships and none of them run its JavaScript. There is one now: the data constants in these tools are capitalised by convention, and every capitalised identifier a tool&rsquo;s script uses must be declared in that script. It catches this exact fault, and the self-test proves it by removing <code>ORDER</code> and confirming the build refuses"),
    ...
    dict(kind="Correction", date="14 Aug 2026", title="Journeyman&rsquo;s Boots drop, and the quest is gone",
         body="Project 1999 text says the boots &ldquo;formerly dropped from Drelzna&rdquo; before being made a quest in October 1999, and that sentence is imported into eqlwiki&rsquo;s <em>Drelzna</em> page where it sits inside a structured NPC record and reads like current data. <strong>They drop.</strong> Reported by our collaborator on 14 August 2026 &mdash; seen dropping for other players, confirmed by guildmates &mdash; and eqlwiki&rsquo;s own item record agrees. Filed <span class=\"tier tC\">TIER C</span>, because we have never killed Drelzna in any session we hold and a first-hand report is not a parse. An auditor grading our survey made exactly this mistake in front of us, reading the classic sentence and not the item record. The survey was right"),
    ...
    dict(kind="Correction", date="17 Aug 2026", title="Tier C withdrawn: a first-hand report is not a tier",
         body="A tier was added on 10 August for named first-hand reports, and it should not have been. "
              "<strong>It was generalised from a single event</strong> &mdash; the site&rsquo;s owner "
              "confirming one thing from a source they trusted &mdash; and one event is not a rank on a "
